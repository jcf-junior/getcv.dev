...
export default function ExperienceInputs({ value, onChange }: Props) {
  ...
  const updateExperience = ( index: number, key: keyof WorkExperience, fieldValue: string, ) => {
  ...
  const [numOfExperiences, setNumOfExperiences] = useState<NumOfExperiences>(value.length);
  ...
  return (
    <section className="mt-8">
      ...
      {value.map((exp, i) => (
        ...
        >
          <button
            className="absolute -top-2 -right-2 bg-red-100 text-red-600 w-6 h-6 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-200"
            onClick={() =>
              numOfExperiences > 1
                ? setNumOfExperiences((prev) => prev - 1)
                : null
            }
          >
          ...
          <label className="flex flex-col font-bold text-xs text-slate-500">
            Start Date
            <input
              defaultValue={exp.startDate ?? "2000-01-01"}
          ...
          <label className="flex flex-col font-bold text-xs text-slate-500 md:cols-span-2">
            ...
            <textarea
              ...
              onChange={(e) => updateHighlights(i, e.target.value.split("\n"))}
